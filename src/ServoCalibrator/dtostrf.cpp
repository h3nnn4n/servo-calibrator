...
// TODO(h3nnn4n): Move this to a lib or something
...
char *my_dtostrf(float val, int8_t char_num, uint8_t precision, char *chr_buffer) {
    ...
    j = 1;
    for (i = 0; i < (char_num - precision - 3); i++)
        j *= 10;

    // Hackish fail-fast behavior for larger-than-what-can-be-printed values, countig the precision + sign ('-') +'.' +
    // '\0'
    if (val >= static_cast<float>(j)) {
        // not enough space
    ...
    i_val = static_cast<int32_t>(val);
    r_val = val - static_cast<int32_t>(i_val);

    // print the integral part ... but it is in reverse order ... so leaves the space for  '.' and the decimal part (and
    ...
    if (precision > 0) {
        ...
        while (precision > 0) {
            // prepare the data
            r_val *= 10.0;
            i_val = static_cast<int>(r_val);
            r_val -= static_cast<float>(i_val);
